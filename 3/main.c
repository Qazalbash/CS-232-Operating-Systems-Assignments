...
char *cd(char *myargs[]) {
    if (myargs[1] == NULL) {
        if (chdir(getenv("HOME")) == -1) perror("chdir() to $HOME failed\n");
...
void history(char *history[]) {
    for (int i = 0; i < MAX; i++)
        if (history[i] != NULL) printf("%d : %s", i, history[i]);
}

void jobs() {
    char *args[] = {"ps", "-A", "-o", "pid,ppid,stat,command", NULL};
    if (execvp(args[0], args) == -1) perror("execvp() failed\n");
}
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        char *myargs[11];
        for (int i = 0; i < 11; i++) myargs[i] = info->CommArray[0].VarList[i];

        if (strcmp(myargs[0], "exit") == 0) return 0;
        ...
        if (pid == -1)
            ...
            if (strcmp(myargs[0], "!CMD") == 0) CMD(info, myargs, H);
